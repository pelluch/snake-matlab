function outputImage = transformImage4D(matrix, inputImg, referenceImage,varargin)
% matrix must the transform from target to source!
%
...
%  'temporalOffset' 
 ...
 for i=1:size(varargin,2)    
    ...
    elseif (strcmp(varargin{i},'customBounds'))
        ...
        i=i+1;
    elseif (strcmp(varargin{i},'temporalScaling'))
        tempScaling= varargin{i+1};
        i=i+1;
    elseif (strcmp(varargin{i},'temporalOffset'))
        tempOffset= varargin{i+1};
        i=i+1;
    ...
    ndims = outputImage.size(4);
    for fr =1:16
    
        for i=1:size(intervals,1)
             ranges([1 3 5]) = intervals(i,:).*chunked_size(1:3)+1;
            ranges([2 4 6]) = min([(intervals(i,:)+[1 1 1]).*chunked_size(1:3) ; outputImage.size(1:3)']);
            ...
            %[x y z]=ndgrid(1:outputImage.size(1), 1:outputImage.size(2), 1:outputImage.size(3));
            % generate all the positions of the output image
            positionInTarget = outputImage.GetPosition([x(:) y(:) z(:) ones(numel(z),1)*fr ]');

             positionInSource = matrix * [positionInTarget(1:3,:) ; ones( 1, size(positionInTarget,2) ) ];
             positionInSource = [positionInSource(1:3,:) ; positionInTarget(4,:)*tempScaling+tempOffset];

             valueInSource =  inputImg.GetValue(positionInSource,mode); % FIX THISSSSSSSSSSSSSSSSSSSSSSSSSSSSSS
            % valueInSource(valueInSource~=valueInSource)=0;% remove NaN
            outputImage.data(ranges(1):ranges(2),ranges(3):ranges(4),ranges(5):ranges(6),fr) = reshape(valueInSource,ranges_size);
            % outputImage.data=reshape(valueInSource,outputImage.size');
        end
    end
    % resample image along time dimension to fit the reference image
