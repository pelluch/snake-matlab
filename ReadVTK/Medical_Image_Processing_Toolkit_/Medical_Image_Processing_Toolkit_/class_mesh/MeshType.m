...
%MESHTYPE  Base class to handle triangulated 3D meshes
%    This class provides basic functionality and structure for triangulated meshes. 
%    In addition to points and triangles, this class  stores attributed per-point or per-triangle.
%
%
%   See also IMAGETYPE, ATTRIBUTETYPE, VIEWMESH.
...
%   This software was designed for research purposes and not for clinical use.
    ...
    methods(Access = public)
        ...
        function v = GetVolume(obj)
            ...
            v = sum(dot(barycentres,normals,2).*areas)/3;
            ...
            for i=pointlist(:)'
                ...
                if numel(x>0)
                    ...
                    for j=unique(neighbors)
                        if numel(neighbors(neighbors==j))<2
                            isBorder(i,:)=1;
                            if i~=j
                                [f1,~] = find(edges==j);
                                [f2,~] = find(edges==i);
