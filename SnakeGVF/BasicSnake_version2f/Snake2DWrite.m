function [P,J]=Snake2D(I,P,Options,fi, timeIndex, folder)
...
f1= figure;
imshow(I,[]); hold on; plot(P(:,2),P(:,1),'b.'); hold on;
export_fig(f1, strcat(folder,'/InitialContour',int2str(timeIndex),'_',int2str(fi),'.png'));
close all;

f1= figure;
imshow(Eext,[]); hold on; plot(P(:,2),P(:,1),'b.'); hold on;
export_fig(f1, strcat(folder,'/ExternEnergy',int2str(timeIndex),'_',int2str(fi),'.png'));
close all;

f1= figure;
[x,y]=ndgrid(1:4:size(Fext,1),1:4:size(Fext,2));
imshow(I), hold on; quiver(y,x,Fext(1:4:end,1:4:end,2),Fext(1:4:end,1:4:end,1)), hold on;plot(P(:,2),P(:,1),'b.'); 
export_fig(f1, strcat(folder,'/ForceField',int2str(timeIndex),'_',int2str(fi),'.png'));
close all;
...
f1= figure;
imshow(I,[]); hold on; 
plot(P(:,2),P(:,1),'r.'); hold on;
export_fig(f1, strcat(folder,'/FinalContour',int2str(timeIndex),'_',int2str(fi),'.png'));
close all;
