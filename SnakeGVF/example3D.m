...
% end
for ind = 12
[V, info] = ReadData3D(strcat('C:/DocsMaracuya/Datos/Snakes/SAguirre/_OriginalReductor_',int2str(ind-1),'.vtk'));
end
...
Options.Lambda = .8; % Weight which changes the direction of the image 
...
ellipseCenter = [160, 130, 8];
majorAxis = 4;
minorAxis = 20;
ellipseAxis = [minorAxis, minorAxis, majorAxis];
theta = 0;
phi = 0;
psi = 0;
...
figure; hold on;
elli = [ellipseCenter minorAxis minorAxis majorAxis  0 0 0];
% drawEllipsoid_jip(elli, 'FaceColor', 'r', ...
%     'drawEllipses', true, 'EllipseColor', 'b', 'EllipseWidth', 2);
% axis equal;
% [faces,vertices] = getEllipsoidMesh(elli);



[Ex,Ey,Ez] = ellipsoid(ellipseCenter(1),ellipseCenter(2),ellipseCenter(3), ellipseAxis(1),ellipseAxis(2),ellipseAxis(3), 30);
% surf(Ex,Ey,Ez);

[v f] = surfToMesh(Ex,Ey,Ez);
...
% tri = delaunay(Ex{:},Ey{:},Ez{:});
tri = delaunay(Ex,Ey,Ez);
fv.faces = tri;

initialP = Snake3D(V,fv,Options);
% plot3(I);
